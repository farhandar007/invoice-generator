...
class InvoiceGenerator:
    def __init__(self, root):
        ...
        Label(self.frame,text="-------Invoice Generator-----",font=("times new roman",30,"bold"),bg="white",fg="blue",bd=0).place(x=50,y=10)
        ...
        # ====submit details
        Button(self.frame, text="Submit Details", command=self.generate_invoice_and_store_data, font=("times new roman", 14),
               fg="white", cursor="hand2", bg="#B00857").place(x=50, y=600, width=180, height=40)

        # email the invoice to customer
        Button(self.frame, text="Send invoice", command=send_mail, font=("times new roman", 14),
    ...
    # here i will save the customers data using pandas later i will save it in a csv file
    # we can append the data to csv
    # to store the customers data i will use pandas
    def store_data(self):
        lst = {"company_name": [self.company_name.get()], "address": [self.address.get()], "city": [self.city.get()],
               "gst": [self.gst.get()], "date": [self.date.get()], "contact": [self.contact.get()], "customer_name": [self.c_name.get()],
               "signature": [self.aus.get()], }
        # Calling DataFrame constructor on list
        dframe = pd.DataFrame(lst)
        print(dframe)
        dframe.to_csv('file1.csv')
        # dframe.to_csv('file1.csv', mode='a', index=False, header=False)


    # here i will make a function that calls both generate_invoice and store_data
    def generate_invoice_and_store_data(self):
        self.generate_invoice()
        self.store_data()


# here we will send the invoice by email to customer using smtplib
# i cannot provide my email to send invoice on mail app password due to privacy reason
# you can just fill the details accordingly
# imp note - it's not your actual email password but the app generated password which you can enable in mail settings
